...
const retryRequest = require('@adobe/aio-lib-core-networking')

const EXPIRY_IN_SECONDS = 'expiryInSeconds'
const BLOB_NAME = 'blobName'
...
class TvmClient {
  ...
  async _getCredentialsFromTVM (url) {
    ...
    const response = await retryRequest.exponentialBackoff(urlWithNamespace, { headers: { Authorization: this.ow.auth } }, this.retryOptions)
    if (response.ok) {
   ...
   * @returns {Promise<object>} credentials for service
   */
  async _getPresignCredentials (endpoint, options) {
    if (!options || !options.expiryInSeconds || !options.blobName || !options.permissions) {
      logAndThrow(new codes.ERROR_MISSING_OPTION({ messageValues: ['missing one or more of required options blobName, expiryInSeconds and permissions'], sdkDetails: options }))
    }

    const newUrl = new URL(urlJoin(this.apiUrl, endpoint, this.ow.namespace))
    newUrl.searchParams.append(EXPIRY_IN_SECONDS, options.expiryInSeconds)
    newUrl.searchParams.append(BLOB_NAME, options.blobName)
    newUrl.searchParams.append(PERMISSIONS, options.permissions)

    const response = await retryRequest.exponentialBackoff(newUrl, { headers: { Authorization: this.ow.auth } }, this.retryOptions)
    if (response.ok) {
      logger.debug(`successfully fetched presign credentials from tvm for ${newUrl}`)
      return response.json()
    }
    const errorBody = await response.text()
    logAndThrow(new codes.ERROR_RESPONSE({
  ...
  /**
   * Revoke the presign credentials from the TVM
   * @param {string} endpoint - TVM API endpoint
   *
   * @private
   * @returns {Promise<object>} success response
   */
  async _revokePresignCredentials (endpoint) {
    const newUrl = new URL(urlJoin(this.apiUrl, endpoint, this.ow.namespace))

    const response = await retryRequest.exponentialBackoff(newUrl, { headers: { Authorization: this.ow.auth } }, this.retryOptions)
    if (response.ok) {
      logger.debug('successfully revoked presign credentials from tvm')
      return response.json()
    }
    const errorBody = await response.text()
    logAndThrow(new codes.ERROR_RESPONSE({
  ...
  /**
   * Request presign credentials for Azure blob storage.
   *
   * ```javascript
   * const tvmResponse = await tvm.getAzureBlobCredentials()
   *
   ...
   * @throws {codes.ERROR_RESPONSE}
   */
  async revokeAzureBlobPresignCredentials () {
    return this._revokePresignCredentials(TvmClient.AzureRevokePresignEndpoint)
  }
}
